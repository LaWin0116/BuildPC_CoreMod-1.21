...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    private static Block getConcreteByColor(DyeColor color) {
        return switch (color) {
    ...
    protected void buildRecipes(RecipeOutput recipeOutput) {
        ...
        for (DyeColor color : DyeColor.values()) {
            ...
            ShapedRecipeBuilder.shaped(
                            RecipeCategory.MISC,
                            ModBlocks.PC_CASES.get(color).get()
                    )
                    .pattern("CCC")
                    .pattern("CGG")
                    .pattern("CCC")
                    .define('C', getConcreteByColor(color))
                    .define('G', Blocks.GLASS)
                    .unlockedBy("has_concrete", has(getConcreteByColor(color)))
                    .unlockedBy("has_glass", has(Blocks.GLASS))
                    .group("pc_case")
                    .save(recipeOutput, ResourceLocation.fromNamespaceAndPath(BuildPCCore.MODID, name));
        }
